...
export default function() {
  ...
  if (decidePostOrGet > 0.95) {
    ...
    if (randNum > 0.8) {
      ...
      let res = http.post(`http://localhost:3010/${Math.floor(Math.random())}/reviews`, formData, { headers: headers });
      check(res, {
        "status was 201": (r) => r.status == 201,
        "transaction time OK": (r) => r.timings.duration < 100
      });
    ...
    if (randNum > 0.8) {
      ...
      let res = http.get(`http://localhost:3010/${Math.floor(Math.random())}/reviews`);
      check(res, {
        "status was 200": (r) => r.status == 200,
        "transaction time OK": (r) => r.timings.duration < 100
      });
      let res2 = http.get(`http://localhost:3010/${Math.floor(Math.random())}/summary`);
      check(res2, {
        "status was 200": (r) => r.status == 200,
        "transaction time OK": (r) => r.timings.duration < 100
      });
